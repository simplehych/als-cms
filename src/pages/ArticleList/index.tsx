...
import { articleQueryAllA } from '../Article/wx_cloud_func';
import { articleDelete, requestA, requestArticleAll } from './model';
...
const handleRemove = async (selectedRows: API.ArticleItem[]) => {
  ...
  try {
    ...
    const ids = selectedRows.flatMap<string>((element) => `${element._id}`)
    console.log("handleRemove ids: ", ids)
    await articleDelete(ids);
    hide();
...
const TableList: React.FC = () => {
  ...
  const columns: ProColumns<API.ArticleItem>[] = [
    ...
    },
    {
      ...
      valueEnum: {
        "online": {
          text: (<FormattedMessage id="pages.articleTable.status.online" defaultMessage="online" />),
          status: 'online',
        },
        "offline": {
          text: (<FormattedMessage id="pages.articleTable.status.offline" defaultMessage="offline" />),
          status: 'offline',
        },
      },
    },
    ...
    },
    {
      ...
      render: (_, record) => [
        ...
        >
          <FormattedMessage id="pages.articleTable.edit" defaultMessage="编辑" />
        </a>
      ],
    },
  ];
